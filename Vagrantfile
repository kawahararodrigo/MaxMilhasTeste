...
Vagrant.configure("2") do |config|
  ...
  config.vm.provision "shell", inline: <<-SHELL
   ...
   # Configurção PostgreSQL
    ...
    # configuração  Zabbix
    sudo sed -i "s|^DBName=zabbix|DBName=zabbix_server|; /^# DBPassword=/a \\\\DBPassword=zabbix" /etc/zabbix/zabbix_server.conf
    sudo sed -i "s|# php_value date.timezone Europe/Riga|php_value date.timezone America/Sao_Paulo|" /etc/apache2/conf-enabled/zabbix.conf
